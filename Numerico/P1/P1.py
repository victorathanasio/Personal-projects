    ...
    for a in range(matrix.cols - 1):
        ...
        if cond:
            ...
            while refcol != matrix.col(a)[line:]:
                ...
                if indi != line + line_:
                    ...
                    multipliers.row_swap(line + line_, indi)
        ...
        if Pivot != 0:
            for i, multi in enumerate(multis):
                ...
                Multi_row = Pivot_row*multis[i]
                Multi_row = Matrix([[arredonda(number, R) for number in Multi_row]])
                Result_row = Target_row - Multi_row
...
def resolve_U(matrix, b, R):
    ...
    for j in range(A.shape[0]-1, -1, -1):
        A_row = A[j, :]
        A_multi = A_row.transpose().multiply_elementwise(x)
        A_multi = Matrix([[arredonda(number, R) for number in A_multi]])
        SUM = arredonda(sum(A_multi), R)
        x[j] = arredonda(b[j] - SUM, R)
        x[j] = x[j]/A_row[j]
        x[j] = arredonda(x[j], R)
    return x
...
class gauss_elimi:
    ...
    def recursi(self):
        ...
        r = r.transpose()
        self.rs.append(r)
        c = resolve_U(self.A, executa_swaps(self.swaps, self.rs[-1])[1], self.R)
        self.cs.append(c)
        x = c + self.xs[-1]
...
class jacobi:
    ...
    def criterios_conv(self):
        ...
        for val in eig:
            val = val.evalf()
            if str(type(val)) != "<class 'sympy.core.add.Add'>":
                vals.append(abs(val))
            else:
                express = val.as_real_imag()
                if express[1] < 0.0000001:
                    vals.append(abs(express[0]))
        maior = max(vals)
        if maior > 1:
...
class gauss_seidel:
    ...
    def criterios_conv(self):
        ...
        self.Beta = abs(Matrix(Beta))
        self.M = max(self.Beta)
